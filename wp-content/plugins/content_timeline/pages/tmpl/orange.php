...
?>
<div class="item <?php if($settings['style']!='default')echo 'my_style_'.$settings['style']?>" data-id="<?php echo $arr['dataid'] ?>" <?php $my_name=(($arr['node-name'] && $arr['node-name'] != '') ? ' data-name="'.$arr['node-name'].'"': ''); echo $my_name;?> data-description="<?php echo  mb_substr($arr['item-title'],0,30);?>">
			<?php echo $image ;?>
		...
		<div class="item_open'<?php ($arr['item-image'] == '' ? ' item_open_noImg' : '') ;?>" data-id="<?php echo $arr['dataid'];?>" <?php (!isset($tpreview) ? 'data-access="'.admin_url( 'admin-ajax.php' ).'?action=ctimeline_frontend_get&timeline='.$id.'&id='.$key.'"': '')?>>
